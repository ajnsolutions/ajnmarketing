...
- **Market Radar** — owner-facing add/remove/prioritize/benchmark controls over the existing `lib/market-context/` competitor provider (expansion, not a rebuild, per `EXISTING_SYSTEM_AUDIT.md`). Persistence foundation shipped (`lib/market-radar/`, migration `037_market_radar.sql`, PR #101). The owner-facing view (Task 002, branch `ai-queue/002-market-radar-view`) has also shipped: `/dashboard/market-radar` (add/remove competitors and benchmarks, reachable via "More tools"). Task 003 (Competitor Observation Engine) shipped on branch `ai-queue/003-competitor-observation-engine`: evidence model, confidence scoring (`lib/competitor-observations/scoring.ts`'s pure `scoreCompetitorSignal`), and tenant-scoped persistence (`lib/competitor-observations/persistence.ts`, migration `038_competitor_observations.sql`) — no UI. It scores `lib/market-context/providers/competitorProvider.ts`'s existing profile-declared signal against the owner's Market Radar tracked competitors; still no live external monitoring anywhere in this repo. **Task 004 (Business Pulse Integration) has now shipped** on branch `ai-queue/004-business-pulse-integration`: `/dashboard/business-pulse` — a first, honest "What Changed" slice reading Task 003's verified observations, each joined to its tracked competitor's name, rendered with a plain-language confidence label (`lib/competitor-observations/confidenceLabels.ts`) and honest source provenance, filterable to high-only/medium-and-above/all (`lib/competitor-observations/display.ts`), reachable via "More tools". This is explicitly not the full Business Pulse (Marketing Health + Growth Momentum) vision — see `docs/project-magic/BUSINESS_PULSE.md`'s implementation-status note. **Task 005 (Weekly Executive Brief: Market Radar section) has now shipped** on branch `ai-queue/005-weekly-executive-brief`: the **weekly** Executive Brief (`weekly_strategy_brief` only) gains `marketRadarHighlights[]` — a new `ExecutiveMarketRadarHighlight` type (`lib/executive-briefing/types.ts`) carrying `observation`/`whyItMatters`/`suggestedAction`/`confidence`, built by the new pure `buildMarketRadarHighlights` (`lib/executive-briefing/buildBrief.ts`), fed by `lib/head-of-marketing/service.ts` fetching `listCompetitorObservationsForUser` once and threading it through `WeeklyBriefingInput.marketRadarObservations`. Always `[]` on the morning brief and monthly report. Rendered in `components/dashboard/executive-brief-section.tsx`'s existing detail-disclosure grid, shown only when non-empty. Marketing Director recommendation evidence integration remains unscoped future work.
