...
**2.0, Wave III (partial)** — Goals & Strategy (goal model, strategy-layer relevance, Goal Progress, goal-aware Growth Advisor briefing), Customer Voice Phase 1 (provider-agnostic intelligence engine, Google Reviews) and Phase 2 (customer-facing experience, Website Testimonials as the second provider; the doc's own "consume intelligence in Growth Advisor" scope is also confirmed live in code — `lib/growth-advisor/buildGrowthAdvisorBriefing.ts` et al. — even though `docs/project-magic/IMPLEMENTATION_ROADMAP.md` still lists it as "next"). **Market Radar — fully shipped across all four originally-planned "How it surfaces" points**: persistence foundation (PR #101), owner-facing view (PR #104), Competitor Observation Engine (PR #107), Business Pulse's "What Changed" view (PR #109), Weekly Executive Brief's Market Radar section (PR #110), Marketing Director recommendation evidence (Task 006). Seasonal Intelligence remains not started (see "Next").
...
Per `docs/project-magic/IMPLEMENTATION_ROADMAP.md`, Wave III's remaining scope — Market Radar is now fully shipped (see "Completed"); what's left:
- **Seasonal Intelligence** — forward-looking, lead-time-aware forecasts. **Investigated for a possible Task 007 (2026-08-08) and explicitly NOT scoped or implemented** — `lib/marketing-memory/seasonality.ts` (a 42-line recurrence classifier) is consumed only by Marketing Memory's own internal Learning-weighting pipeline, unrelated to any owner-facing forecast; the actual "seasonal hint" shown today (`lib/head-of-marketing/service.ts`'s `seasonalCampaign`) comes from an entirely separate, architecturally disconnected source — an OpenAI-generated Marketing Plan field with no evidence trail or confidence level, occupying the exact product surface `docs/project-magic/SEASONAL_INTELLIGENCE.md` describes. Real providers already exist for several of the doc's six described inputs (weather, search trends, holidays, local events) but nothing wires any of them together. Implementing this for real requires a genuine product decision — which input(s), which surface(s), relationship to the existing hint — not currently resolved by any doc or code. See `OPEN_ITEMS.md`'s "Task 007 stop condition" for full detail. **Do not implement Seasonal Intelligence without that decision** — a narrower scope was deliberately not invented just to produce a task.
