...
Related, one level up: `.ai/runs/` holds per-execution logs and summaries (`scripts/ai/run-queue.ts` writes these); `.ai/exports/` holds the human/ChatGPT-facing exports (`scripts/ai/export-memory.ts` and `scripts/ai/generate-morning-brief.ts` write these).
