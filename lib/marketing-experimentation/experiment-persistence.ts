...
import type { PlannedExperimentDraft } from "@/lib/marketing-experimentation/experiment-planner";
import {
...
function mapExperimentRow(row: Record<string, unknown>): MarketingExperiment {
  return {
    ...
    outcome: (row.outcome as ExperimentOutcome) ?? emptyExperimentOutcome(),
    metrics: (row.metrics as ExperimentMetrics) ?? emptyExperimentMetrics(),
