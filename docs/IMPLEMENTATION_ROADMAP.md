...
- **Shipped (Guided Onboarding & Setup — Phase 3B):** Post-onboarding setup checklist and progress (extends Phase B without replacing Magic wizard). See [`GUIDED_ONBOARDING_AND_SETUP.md`](./GUIDED_ONBOARDING_AND_SETUP.md).
...
- **Shipped (Marketing Experimentation Engine — Phase 2E):** Experiments may only be created by approving a persisted, server-authored proposal — Marketing Director's deterministic eligibility rule evaluates recommendations and writes proposals server-side only (no client-supplied decision key of any kind); user approval converts a proposal into exactly one experiment; deterministic lifecycle; measurement reports an honest aggregate over the KPI window and never fabricates a per-variant winner (existing analytics have no per-variant attribution); HoM Experiments section (Proposed/Active/Completed); completion records Marketing Memory observations (not learnings). See [`MARKETING_EXPERIMENTATION_ENGINE.md`](./MARKETING_EXPERIMENTATION_ENGINE.md).
- **Shipped (Decision Intelligence & Learning Impact — Phase 2F):** Read-only explanation/traceability layer — durable Marketing Director decision snapshots (`marketing_memory_decision_links`), explicit evidence traces (never fuzzy title matching), deterministic decision comparison and "why the plan changed" explanations, honest experiment/campaign influence reporting, HoM preview + dedicated `/dashboard/decision-intelligence` page, Interactive HoM question categories, informational-only Strategic Calendar events. Never creates recommendations/campaigns/experiments or writes Marketing Memory. See [`DECISION_INTELLIGENCE_AND_LEARNING_IMPACT.md`](./DECISION_INTELLIGENCE_AND_LEARNING_IMPACT.md).  
...
- **Shipped (Pilot Validation & Go-Live Readiness — Phase 6):** Operator Pilot Validation page — readiness audit, operational path validation, journey checklist, production readiness report, observability aggregates, and [`PILOT_VALIDATION_GUIDE.md`](./PILOT_VALIDATION_GUIDE.md). Compose-only over Phase 5 / ops / readiness; cron gate remains closed.  
