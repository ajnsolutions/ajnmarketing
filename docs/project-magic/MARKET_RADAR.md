...
The Marketing Director recommendation-evidence surfacing layer has now also shipped (Task 006, 2026-08-08): `ClientRecommendationDecisionPackage` gains a `competitorEvidence: ClientCompetitorEvidence[]` field (`lib/recommendation-presentation/types.ts`), built by the new `buildCompetitorEvidence` (`lib/recommendation-presentation/competitorEvidence.ts`) — real, current, sufficiently-confident competitor observations shown as general business-level competitive context alongside a recommendation, never claimed as that recommendation's specific cause (there is no opportunity-detection logic anywhere in this codebase linking a specific recommendation to a specific competitor, and none was added). Rendered in the Approval Center's existing "Why this recommendation" disclosure (`components/dashboard/approval-queue.tsx`) only when non-empty. See `docs/CLIENT_RECOMMENDATION_EXPERIENCE.md` §4/§13 for the full reasoning.

**All four of this doc's originally-planned "How it surfaces" points are now shipped**: the dedicated Market Radar view (Task 002), Business Pulse's "What Changed" view (Task 004), the Weekly Briefing's Market Radar section (Task 005), and Marketing Director recommendation evidence (Task 006). What remains unbuilt is the deeper vision each of these deliberately did not attempt: live external competitor monitoring (still none anywhere in this codebase, by design — every surfacing layer scores the same one real signal source, `lib/market-context/providers/competitorProvider.ts`'s profile-declared data) and Business Pulse's full Growth Momentum composition (Wave IV, gated on Waves I–III producing real production signal first).
