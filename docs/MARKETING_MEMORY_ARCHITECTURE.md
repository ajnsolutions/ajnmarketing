...
**Phase 1 — Observation and evidence foundation. IMPLEMENTED** — see [`MARKETING_MEMORY_FOUNDATION.md`](./MARKETING_MEMORY_FOUNDATION.md) for the full as-built record. Shipped `marketing_memory_observations`, `marketing_memory_context_snapshots`, and (elevated from its original Phase 2 scope, anchored on observations instead of a not-yet-existing Learning) `marketing_memory_evidence_links`. Observations are recorded via two best-effort, non-blocking hooks into already-authoritative code — `recommendation_outcome_events` insertion and `analytics_snapshots` capture — not a background job reading those tables after the fact. No Learnings, no customer-visible change. The retention exit criterion below was addressed with a `retention_classification` column plus a hard `expires_at` boundary on context snapshots and a read-only diagnostic query for expired candidates — no cleanup job is scheduled or activated. **Exit criterion (added by the self-review in §24):** a retention/compaction policy for these two append-only tables must be designed and implemented before Phase 1 is considered complete — unbounded growth must not ship silently. *(Satisfied: retention classification + expiry boundary shipped; an actual archival/compaction job remains future work, tracked in `MARKETING_MEMORY_FOUNDATION.md` §15.)*

**Phase 2 — Learnings and confidence. IMPLEMENTED** — see [`MARKETING_MEMORY_LEARNINGS.md`](./MARKETING_MEMORY_LEARNINGS.md) for the full as-built record. Shipped `marketing_memory_learnings` and a nullable `learning_id` column (plus `contribution`) on the Phase 1 `marketing_memory_evidence_links` table (alongside `observation_id`, not replacing it — every Phase 1 row remains valid). Implemented exactly two learning families (`timing_performance`, `recommendation_action_outcome`) — intentionally limited to what Phase 1 observations can honestly support; weather and event-attention families remain unimplemented since Phase 1 never classifies context `impact_direction` beyond `'unknown'`. The confidence rule set (§10) is implemented as a pure, tested function in `lib/marketing-memory/learningMath.ts`, with every threshold centralized in `lib/marketing-memory/learningConfig.ts`. Learnings are evaluated only via a manually-invoked admin route (`POST /api/admin/trigger-marketing-memory-learning-evaluation`), never per-request and never wired into Phase 1's observation-ingestion hooks. Still no customer-visible change — Learnings exist but nothing consumes them yet, and no Marketing Director path was touched.

**Phase 3 — Customer preferences and overrides. IMPLEMENTED** — see [`MARKETING_MEMORY_PREFERENCES.md`](./MARKETING_MEMORY_PREFERENCES.md). Shipped `marketing_memory_preferences` and `marketing_memory_overrides`, actor-attributed writes, soft supersession, permanent-override promotion, precedence helpers (read-time vocabulary only), authenticated APIs, and a minimal Settings → Marketing preferences surface. Explicitly **not** wired into Marketing Director or recommendation behavior — that remains Phase 4.

**Phase 4 — Marketing Director consumption. IMPLEMENTED** — see [`MARKETING_DIRECTOR_MEMORY_INTEGRATION.md`](./MARKETING_DIRECTOR_MEMORY_INTEGRATION.md). `MarketingMemoryEvidencePackage` is built upstream and passed as optional `memoryEvidence` into `resolveMarketingDirectorDecision`. Precedence is ordinal (prohibitions → preferences → goals → strong/developing/early learnings → market context → existing package order). Cold-start / null evidence preserves prior decision selection. `marketing_memory_decision_links` (persist decision audit rows) remains a later additive step — not required for consumption.
