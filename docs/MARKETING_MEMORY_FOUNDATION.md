...
Per the phased plan in `MARKETING_MEMORY_ARCHITECTURE.md` §20, the next phase is **Phase 2 — Learnings and confidence**: add `marketing_memory_learnings` and add a `learning_id` column to `marketing_memory_evidence_links` (nullable, alongside the `observation_id` this PR shipped — not a replacement), implement the confidence rule set as a pure, tested function, and compute Learnings periodically (not per-request) from this PR's observations. No customer-visible change is expected in Phase 2 either — Learnings will exist but nothing will consume them until Phase 4.
