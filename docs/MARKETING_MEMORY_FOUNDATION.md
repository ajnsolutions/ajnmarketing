...
**Phase 2 — Learnings and confidence** and **Phase 3 — Customer preferences and overrides** are now implemented — see [`MARKETING_MEMORY_LEARNINGS.md`](./MARKETING_MEMORY_LEARNINGS.md) and [`MARKETING_MEMORY_PREFERENCES.md`](./MARKETING_MEMORY_PREFERENCES.md). Per the phased plan in `MARKETING_MEMORY_ARCHITECTURE.md` §20, the next phase is **Phase 4 — Marketing Director consumption**. Learnings and preferences exist but are not consulted by `resolveMarketingDirectorDecision` until that phase.
