...
4. Enable the two example tasks in `RUN_QUEUE.yaml` (change their `status` from `disabled` to `pending`) in a throwaway local copy or branch, run `npm run ai:queue`, and confirm: two real PRs open, in the right order, each with the `.ai/` memory updates present, each passing quality gates. Then revert the example tasks back to `disabled` (they are documentation-only demonstrations, not meant to stay enabled) before merging anything to `main`.
